	...
	std::string layers_regex) 
{
	...
	{
		reporter.StartEpoch(epoch, optim_no_bn.options.learning_rate()/*learning_rate_*/);
		// Reset data loaders
#ifdef NDEBUG
...
std::tuple<float, float, float, float, float, float> MaskRCNNImpl::ValidEpoch(
	...
	torch::data::StatelessDataLoader<CocoDataset, torch::data::samplers::RandomSampler>&	datagenerator,
	/*torch::data::DataLoader<CocoDataset, torch::data::samplers::RandomSampler>&	datagenerator,*/
	uint32_t steps) 
{
	float loss_sum = 0;
...
std::string MaskRCNNImpl::GetCheckpointPath(uint32_t epoch) const 
{
	return (fs::path(model_dir_) /
		("checkpoint_epoch_" + std::to_string(epoch) + ".pt")).string();
}
